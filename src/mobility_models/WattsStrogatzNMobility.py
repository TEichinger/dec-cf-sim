...
class WattsStrogatzNMobility(MobilityTemplate):
	""" Class that defines a mobility model on the basis of a Watts-Strogatz small-world random graph. It builds a graph according to Barabas and Albert'sample
	...
	# see https://networkx.github.io/documentation/stable/reference/generated/networkx.generators.random_graphs.newman_watts_strogatz_graph.html#networkx.generators.random_graphs.newman_watts_strogatz_graph
	...
	Input:
		...
		- pairwise		: <float> The probability of adding a new edge for each edge
	...
	def __init__(self, graph, N, T, k, p, graph_seed = None, WS_graph = None):
		...
		self.mobility_string	= "WattsStrogatzN"
		self.WS_graph			= WS_graph
		self.n					= len(self.graph.peers())
		self.k					= k # each node is initially connected with k other nodes (nearest neighbors)
		self.p					= p # edges are rewired with probability p
	...
	def generate_WS_graph(self):
		""" Build a Watts_Strogatz random graph as a networkx graph. """
		self.WS_graph = nx.newman_watts_strogatz_graph(self.n, self.k, self.p, self.graph_seed)
	...
	def generate_graph(self):
		...
		for t in range(self.T):
			...
			contacts_indices, random_gen = self.sampleN(random_gen, self.num_edges_per_epoch, list(self.WS_graph.edges()))
