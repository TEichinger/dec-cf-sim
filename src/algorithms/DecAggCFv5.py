...
class DecAggCFv5(CollectPayloadTemplate, AggregatePayloadTemplate, CollectGarbadgeTemplate, DecAlgoTemplate):
	""" This class is meant to implement an altered version of the the aggregating (!) decentralized Collaborative Filtering (CF) algorithm (cf. Eichinger and Westerkamp 2020)
	where the formulae of the aggregated rating and similarities are deprived of the similarities of child_children to the parent are removed. More precisely:

	In v5, we abstract from DecAggCFv2 in the sense that we do not sort by the similarity of the parent to the child_children, yet
	take the similarity of the child_children to the child as representative similarities. In order to circumvent a large bias by the
	perfect similarity of child to itself, we will keep the similarity of child to parent instead of calculating the similarity of child to itself.
	The amendments are marked with (o) [only changes in the <collect_payload> method are required.

	agg_sim_{v->u} * agg_rating_{v->u}	= sum_W sim * rating	+ sum_Omega agg_sim * agg_rating
	agg_sim_{v->u}						= sum_W sim				+ sum_Omega agg_sim

	instead of (in the original version)

	agg_sim_{v->u} * agg_rating_{v->u}	= sum_W sim*rating	+ sum_Omega sim * agg_sim * agg_rating
	agg_sim_{v->u}						= sum_W sim			+ sum_Omega sim * agg_sim

	In particular: Differences to the DecAggCF class are in: (see the positions where (x) in the below code)
		- the <aggregate_payload> method
	...
	def __init__(self, graph, train_df, snapshot_info_string = "", dataset_string = "", initial_snapshot_string = "", output_dir =  os.path.join(root_dir,"data/snapshots"),\
		...
		self.algo_string 				= "DecAggCFv5"
		# for selection of data to be exchanged
		self.max_agg_sim				= max_agg_sim

		# initialize <aggregate_payload> method
		#AggregatePayloadTemplate.__init__(self)
		# initilizate <collect_garbadge> method
		#CollectGarbadgeTemplate.__init__(self)
...
if __name__ == "__main__":
	...
	if mobility_string == "AssignNMobility":
		...
		mobility_model	= mobility_model(graph, N, T, graph_seed=graph_seed)
		save_every_i = 5
	elif mobility_string == "DirectNMobility":
		mobility_model = DirectNMobility
		mobility_model	= mobility_model(train_df, N, T, sim_string = sim_string, min_sim = min_sim_to_sender, sim_dict = sim_dict, graph_seed = graph_seed)
		save_every_i = 5
		mobility_string = "DirectNMobility"
	elif mobility_string == "NeighborhoodFormationNMobility":
		mobility_model = NeighborhoodFormationNMobility
		mobility_model	= mobility_model(train_df, N, sim_string = sim_string, sim_dict = sim_dict)
		save_every_i = 1
		mobility_string = "NFNMobility" # shorter name
	elif mobility_string == "UniformRandomNMobility":
		mobility_model = UniformRandomNMobility
		T = 1
		mobility_model	= mobility_model(graph, N, T, graph_seed=graph_seed)
		save_every_i = 1
		...
		mobility_model = BarabasiAlbertNMobility
		mobility_model = mobility_model(graph, N, T, m, graph_seed = graph_seed)
		save_every_i = 5
		...
		mobility_model = WattsStrogatzNMobility
		mobility_model = mobility_model(graph, N, T, k, p, graph_seed = graph_seed)
		save_every_i = 5
	...
	algorithm = DecAggCFv5(graph, train_df, snapshot_info_string = snapshot_info_string, dataset_string = dataset_string, initial_snapshot_string = initial_snapshot_string, output_dir = output_dir, sim_string = sim_string,
