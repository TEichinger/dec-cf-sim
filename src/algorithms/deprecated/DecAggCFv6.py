...
class DecAggCFv6(CollectPayloadTemplate, AggregatePayloadTemplate, CollectGarbadgeTemplate,DecAlgoTemplate): #
	...
	without aggregated similarities. The idea is that the similarities between the parent and the chil_children are taken for every item instead of the child_child-and-item-specific aggregated similarities.
	The profile aggregation can be reduced to a single formula, which resembles a typical user-based CF formula.
	NOTE that the set W are the child_children.

	agg_rating_{v->u}	= sum_W sim(w,u) * agg_rating	/  sum_W sim(w,u)

	instead of (in the original version)

	agg_sim_{v->u} * agg_rating_{v->u}	= sum_Omega sim*rating	+ sum_Omega sim * agg_sim * agg_rating
	agg_sim_{v->u}						= sum_Omega sim			+ sum_Omega sim * agg_sim
...
if __name__ == "__main__":
	...
	algorithm = DecAggCFv6(graph, train_df, snapshot_info_string = snapshot_info_string, dataset_string = dataset_string, initial_snapshot_string = initial_snapshot_string, output_dir = output_dir, sim_string = sim_string,
