...
class DecShCFv2(CollectPayloadTemplate, AggregatePayloadTemplate, CollectGarbadgeTemplate, DecAlgoTemplate):
	...
	In version2, we implement a version of the aggregation mechanism in Shokri et al. (2008). The aggregation happens as follows:
	...
	The graph has to be built with a mobility model. Then the method <DecAlgoTemplate.fill_snapshots>/<DecAlgoTemplate.fill_snapshots_inparallel>
	will successively calculate the snapshots until a final time period T. At given time intervals the snapshots will be able to be saved.
...
if __name__ == "__main__":
	...
	algorithm = DecShCFv2(graph, train_df, snapshot_info_string = snapshot_info_string, dataset_string = dataset_string, initial_snapshot_string = initial_snapshot_string, output_dir = output_dir, sim_string = sim_string,
